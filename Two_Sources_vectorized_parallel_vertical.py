...
# -*- coding: utf-8 -*-
"""
Vertical two-source configuration (stacked along +y) using Mathieu-series.
"""
...
def Ye(order, eta):
    """Even radial Mathieu (2nd kind), real part."""
    return m.Ke(order, eta).real


def Yo(order, eta):
    """Odd radial Mathieu (2nd kind), real part."""
...
def F_target(x, Ci):
    """
    Boundary target function and metadata for Ci at boundary coordinate x.
    """
    if Ci > 0:
        return (Ci*gamma + Ca) * np.exp(-beta * x), Ci, 'r'
    else:
        # original used a negative form for acceptor
        return -(abs(Ci)*gamma + Ca) * np.exp(-beta * x), Ci, 'r'


def c(x, y):
    """
    Concentration at (x,y) for vertical pair at (0,0) and (D1,D2).
    """
    if (x**2 + y**2) <= r**2:
        return C0
    ...
    # original vertical code used always-gamma in both branches for acceptor
...
def Conc_array(x_min, x_max, y_min, y_max, inc):
    """
    Compute (xaxis, yaxis, Z) for domain with spacing inc.
    """
    xaxis = np.arange(x_min, x_max, inc, dtype=float).round(2)
    yaxis = np.arange(y_min, y_max, inc, dtype=float).round(2)
...
def run(
    *,
    ...
    M_param=M,
    D1_param=0.0,
    D2_param=None,         # default becomes 2*r + 0.2 if None
    x_min=0.0, x_max=600.0, y_min=-5.0, y_max=None, inc=inc_default
):
    """
    Execute vertical two-source simulation and plot.
    D2 defaults to 2*r + 0.2. y_max defaults to r + 2*inc (as in original).
    """
    global r, alpha_l, alpha_t, beta, C0, C1, Ca, gamma, n, M, d, q, D1, D2, m, Coeff
    ...
    M       = int(M_param)
    D1      = float(D1_param)
    D2      = (2.0*r + 0.2) if D2_param is None else float(D2_param)
    ...
    plt.xticks(range(len(xaxis))[::max(1, int(50/inc))], xaxis[::max(1, int(50/inc))])
    plt.yticks(range(len(yaxis))[::max(1, int(1/inc))],  yaxis[::max(1, int(1/inc))])
